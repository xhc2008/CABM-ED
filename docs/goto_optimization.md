...
#### 情况2：暂存goto（随机判定成功）
- **触发条件**：不在冷却中，且随机判定成功（基于回复意愿计算的成功率）
- **行为**：
...
#### 情况3：立即触发（随机判定失败）
- **触发条件**：不在冷却中，且随机判定失败（基于回复意愿计算的成功率）
- **行为**：
  - 按照原逻辑，直接结束对话并移动到新场景
  - 设置goto冷却时间
- **效果**：角色不想继续聊天，直接离开
...
**成功率示例**：
- 当前回复意愿 = 100：成功率 = 70%，70%概率暂存goto，30%概率立即移动
- 当前回复意愿 = 50：成功率 = 20%，20%概率暂存goto，80%概率立即移动
- 当前回复意愿 = 30：成功率 = 0%，100%概率立即移动
- 当前回复意愿 = 0：成功率 = 0%（最低），100%概率立即移动

**判定逻辑**：
- 随机值 < 成功率 → 判定成功 → 情况2（暂存goto）
- 随机值 >= 成功率 → 判定失败 → 情况3（立即移动）
...
- **新逻辑**：
  - 冷却中：0%概率移动（直接抛弃）
  - 不在冷却中：根据回复意愿随机判定
    - 回复意愿 = 100：70%概率暂存（情况2），30%概率立即移动（情况3）
    - 回复意愿 = 50：20%概率暂存，80%概率立即移动
    - 回复意愿 <= 30：0%概率暂存，100%概率立即移动
...
- `ChatDialog: 随机值: Z, 判定: 愿意留下/想要离开`
- `ChatDialog: goto在冷却中，抛弃goto字段`
- `ChatDialog: 角色愿意留下，暂存goto字段`
- `ChatDialog: 角色想要离开，立即触发场景变化`
