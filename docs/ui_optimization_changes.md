...
**更改**:
- 在 `load_scene()` 函数中添加场景变化检测
- 只有在场景ID真正改变时才调用 `character.load_character_for_scene(scene_id)`
- 当只是改变时间段或天气时，只更换背景图片，不重新加载角色位置

**效果**:
- 在侧边栏调整时间段（白天/黄昏/夜晚）或天气（晴天/雨天/雷雨）时，角色保持在当前位置
- 只有切换到不同的场景（如从客厅到卧室）时，才会重新加载角色位置
- 提供更流畅的用户体验，避免角色位置不必要的重置
...
1. 启动游戏，进入有角色的场景
2. 记住角色的当前位置
3. 展开侧边栏，切换时间段（如从白天到夜晚）
4. 确认背景图片改变，但角色位置保持不变
5. 切换天气（如从晴天到雨天）
6. 确认背景图片改变，但角色位置保持不变
7. 切换到不同场景（如从客厅到卧室）
8. 确认角色位置被重新加载到新场景的预设位置

### 测试自动时间
1. 启动游戏
2. 展开侧边栏，确认"自动调整时间"选项已勾选
3. 观察场景时间段是否与当前系统时间匹配
4. 确认角色位置没有因为自动时间调整而重置
5. 取消勾选，手动选择时间段，确认能正常切换
6. 重新勾选，确认自动时间功能恢复
