...
signal costume_selector_requested

# 判定区域（隐藏的点击区域）
var click_area: Control
# 选项菜单
var options_panel: Panel
var costume_button: Button

const ANIMATION_DURATION = 0.2
const ELEMENT_ID = "costume_button"  # 配置文件中的ID
...
func _create_options_panel():
	...
	# 换装按钮
	costume_button = Button.new()
	costume_button.text = "👗 换装"
	costume_button.pressed.connect(_on_costume_button_pressed)
	vbox.add_child(costume_button)
...
func _on_costume_button_pressed():
	"""换装按钮点击"""
	costume_selector_requested.emit()
	hide_menu()
